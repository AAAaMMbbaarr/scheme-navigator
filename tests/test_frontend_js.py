...
@pytest.mark.skipif(not (NODE and JSDOM), reason="needs node + `npm install` (jsdom)")
def test_profile_values_are_displayed_in_the_selected_language():
    r = subprocess.run(["node", "--test", "tests/profile_values.test.js"], cwd=ROOT, capture_output=True, text=True, timeout=180)
    assert r.returncode == 0, r.stdout[-3000:] + r.stderr[-1000:]


@pytest.mark.skipif(not NODE, reason="node not installed")
@pytest.mark.parametrize("f", ["app.js", "voice.js", "strings.js", "values.js"])
